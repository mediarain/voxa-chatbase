...
import voxaChatbase from "../src/Voxa-Chatbase";
import { IVoxaChatbaseConfig } from "../src/Voxa-Chatbase";
import views from "./views";

const CHATBASE_URL = "https://chatbase-area120.appspot.com";
const chatbaseConfig: IVoxaChatbaseConfig = {
  apiKey: "some_api_key",
  platform: "alexa"
};
...
describe("Voxa-Chatbase plugin", () => {
  ...
  it("should register ChatbaseAnalytics on unexpected error", async () => {
    ...
    const spy = simple.spy(() => ({
      say: "BadInput.RepeatLastAskReprompt.say",
      to: "invalid-state"
    }));
    voxaApp.onError(spy);
